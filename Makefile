...
EXAMPLE_DEPENDS:='examples-requirements'
...
setup-examples: $(VENV_NAME) $(VENV_NAME)/bin/python setup-venv local-install
	...
	$(QUIET)test -f docs/examples/$(EXAMPLE_DEPENDS).txt || exit 32 ;
	$(VENV_NAME)/bin/python -B -m pip $(PIP_PREFIX_FLAGS) install $(PIP_COMMON_FLAGS) -r docs/examples/$(EXAMPLE_DEPENDS).txt
