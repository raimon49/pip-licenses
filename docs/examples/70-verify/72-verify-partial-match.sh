...
#
# This example demonstrates partial (substring) matching for --fail-on and --allow-only.
...
CMD_LINE_TEXT_P=$(printf 'pip-licenses %s' "--partial-match --allow-only=\"MIT;BSD\"" ;)

# Run the example - partial match with allow-only
run_command "${CMD_LINE_TEXT_P}"
pause 2.2
